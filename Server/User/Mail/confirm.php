...
if (isset($_GET['key'])) {
    ...
    $data = mysqli_query($db, "SELECT * FROM `DB` WHERE `Username` = '{$inf[0]}' AND `Password` = '{$inf[1]}'");
    if ($data->num_rows > 0 && checkData($inf[0], $inf[1], $inf[2])) {
        $row = mysqli_fetch_array($data);
        if ($row['Email'] == 'null') {
            mysqli_query($db, "UPDATE `DB` SET `Email` = '{$inf[2]}' WHERE `Username` = '{$inf[0]}' AND `Password` = '{$inf[1]}'");
            echo 'OK';
        }
        else
            echo "EXISTS";
