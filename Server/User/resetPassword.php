...
if (isset($_GET['key'])) {
    ...
    $data = mysqli_query($db, "SELECT * FROM `FTL` WHERE `Username` = '{$inf[0]}' AND `Email` = '{$inf[1]}'");
    if ($data->num_rows > 0 && checkData($inf[0], $inf[2], $inf[1])) {
        $row = mysqli_fetch_array($data);
        mysqli_query($db, "UPDATE `FTL` SET `Password` = '{$inf[2]}' WHERE `Username` = '{$inf[0]}' AND `Email` = '{$inf[1]}'");
        echo 'OK';
    } else
        echo 'INCORRECT'; 
    mysqli_close($db);
